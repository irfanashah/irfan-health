...
- **Cron monitoring / alerting** — `/api/cron/whoop` runs every 6 h and writes its result to `ingestion_log`. There's no alert if it starts failing silently (Whoop API change, refresh token revoked, Supabase quota, etc.) — only a manual check of `ingestion_log` surfaces it. Worth a small "last successful run > 24 h ago → notify" job before this gets relied on for clinical decisions.
