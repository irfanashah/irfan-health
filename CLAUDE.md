...
- **Slice 2 manual hand-off:** add `WITHINGS_CLIENT_ID` + `WITHINGS_CLIENT_SECRET` to Vercel env vars → deploy → OAuth handshake at `https://irfan-health.vercel.app/api/auth/withings` → verify row in `oauth_tokens` for `source_slug = 'withings'` → run first refill (POST `/api/refill/withings` or re-import `RefillButton`) → confirm Diagnose gap = 0 → wait for first cron tick at 00:00/12:00 UTC and verify a `success` row in `ingestion_log`. Steps 11–15 of `slice-2-withings-bp-spec.md`.
...
- **Withings `buildBpRow` is the single source of truth for "what's a valid BP row"**, exported from `adapters/withings/index.ts` and imported by both the cron adapter (via the `withingsAdapter.fetchAndIngest` loop) and `/api/refill/withings`. Avoids drift (gotcha #24).
