...
**Also pending from this session (Irfan, manual):** re-run `migration_003` in Supabase to pick up the H4 fix — confirm `SELECT last_meal_to_sleep_min FROM daily_metrics WHERE date = CURRENT_DATE` returns NULL, not 0, for today.
