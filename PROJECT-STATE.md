...
## Now
Oxylink desaturation slice shipped: parser now emits `spo2_odi` (3% AASM-style, screening-grade) + `spo2_time_below_90_pct` daily-summary rows plus one `spo2_overnight_curve` row/night (display-downsampled SpO2 array packed into `extras`). All clinical numbers computed on the full ~4 s in-memory series; only the stored curve is downsampled. Migrations 003 + 004 extended (both `CREATE OR REPLACE`). New `Overnight Oxygen` Dashboard panel (4-stat readout + distribution donut + overnight trace + range trend); SpO2 card **removed from Recovery & Sleep** so SpO2 lives in exactly one place. ODI + time-below-90 flow into the Baselines & drift tab automatically (12 signals now). `npm run build` clean — 27 routes. **Pending:** re-run migrations 003 + 004 in Supabase, re-drop one Oxylink CSV to backfill the new metrics + curve for the 5 nights already ingested (idempotent via `(source_slug, source_record_id)`), then eyeball the deploy.
...
3. **Eyeball the deploy** at `https://irfan-health.vercel.app/`: Dashboard tab should now contain the new "Overnight Oxygen" panel between Recovery & Sleep and Weight (last-night readout + distribution donut + overnight curve trace + min-SpO2/ODI range trend). Recovery & Sleep should no longer carry the SpO2 section. Baselines & drift tab should list 12 metrics (the 10 from before + `spo2_odi` + `spo2_time_below_90`), with both new ones `establishing` until ~7–12 nights accrue.
...
- **Desaturation event markers on the Overnight Oxygen trace** — the parser doesn't store individual event timestamps (only the count); rendering markers off the downsampled display curve would be inaccurate vs the full-res events. Add only if we start storing event timestamps too.
- **Today-at-a-glance min-SpO2 KPI tile** — flagged in the desat spec, deferred (would reflow the 6-wide KPI grid; easy add later).
